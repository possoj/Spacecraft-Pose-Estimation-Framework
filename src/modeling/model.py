...
from src.modeling.head.keypoints import KeypointRegressionHead
...
def import_model(
    ...
    """
    Import and configure the Brevitas model.

    Args:
        data: Dictionary containing the dataset.
        backbone_name: Name of the backbone model.
        head_name: Name of the head model.
        params_path: Path to the model parameters.
        bit_width_path: Path to the bit-width configuration file.
        manual_copy: Flag to manually copy state dictionary.
        in_channels: Number of input channels.
        batchnorm: Flag to use batch normalization.
        residual: Flag to use residual connections.
        quantization: Flag to use quantization.
        ori_mode: Mode of orientation (e.g., 'regression', 'classification', 'keypoints').
        n_ori_bins: Number of orientation bins in orientation branch (if classification).
        pos_mode: Mode of position (e.g., 'regression', 'classification', 'keypoints').
        n_pos_bins: Number of position bins in position branch (if classification).

    Returns:
        Tuple containing the model and the layer-wise bit-width configuration.
    """
    assert ori_mode in ['classification', 'regression', 'keypoints']
    assert pos_mode in ['classification', 'regression', 'keypoints']

    if ori_mode == 'classification':
        assert n_ori_bins is not None

    if pos_mode == 'classification':
        assert n_pos_bins is not None

    # Extract example image and target for future example
    image, target = next(iter(data[list(data.keys())[0]]))

    # Load bit-width if using Brevitas model
    if 'brevitas' in backbone_name or 'brevitas' in head_name:
        if params_path is not None:
            assert bit_width_path is not None, \
                'The bit_width_path of a brevitas model must be specified if parameters are loaded'
    ...
    # Configure the backbone of the neural network
    backbone_map = {
    ...
    # Backbone parameters (depending on the neural network and parameters)
    last_return_quant = 'brevitas' in backbone_name and 'brevitas' in head_name and quantization
    backbone_config = {
    ...
    # Configure the head of the neural network
    head_map = {
        'ursonet_brevitas': QURSONetHead,
        'ursonet_pytorch': URSONetHead,
        'keypoints_regression_pytorch': KeypointRegressionHead,
    }
    ...
    if ori_mode == 'keypoints':
        n_outputs = target['keypoints'].size()[-1]
        head_config = {
            'n_outputs': n_outputs,
        }
    else:
        head_config = {
            'n_ori_outputs': 4 if ori_mode == 'regression' else n_ori_bins,
            'n_pos_outputs': 3 if pos_mode == 'regression' else n_pos_bins,
        }
    head_config.update(common_head_config)

    # Add optional parameters for quantization
    if 'brevitas' in head_name:
        img_size = (image['torch'].size(2), image['torch'].size(3))
        ...
        head_config.update({
            'pool_kernel': avg_pooling_size[img_size],
    ...
    if params_path is not None:
        ...
        if 'mobilenet_v2' in backbone_name:
            print(f"Load weights...", end='')
            try:
                model.features.load_state_dict(copy_state_dict(
                    torchvision.models.mobilenet_v2(pretrained=True).features.state_dict(), model.features.state_dict())
                )
                print("OK")
