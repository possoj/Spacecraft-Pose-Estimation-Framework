...
class SPEFinn:
    ...
    def predict_finn(self, image: torch.Tensor):
        ...
        pose = self.spe_utils.last_activ({ori_key: ori.detach().cpu().numpy(), pos_key: pos.detach().cpu().numpy()})
    ...
    def predict_and_compare(self, image: torch.Tensor, true_pose, print_results=False):
        ...
        if print_results:
            ...
            similarity = torch.isclose(out_finn_features, out_torch_features, atol=1e-6, rtol=1e-6)
