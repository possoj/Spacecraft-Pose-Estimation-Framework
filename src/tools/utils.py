...
def set_seed(seed: int = 1) -> None:
    ...
    torch.use_deterministic_algorithms(False)  # Should be True, but issues when true with forward pass on a quantized model
...
def save_score_error(
    ...
    """
    with pd.ExcelWriter(os.path.join(path, name), engine='xlsxwriter') as writer:
