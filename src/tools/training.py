...
def train(model: torch.nn.Module,
    ...
    # List of targets (labels) to send on GPU
    # if 'keypoints' in (spe_utils.ori_mode, spe_utils.pos_mode):
    #     gpu_list = ['keypoints']
    # else:
    #     gpu_list = ['ori'] if spe_utils.ori_mode == 'regression' else ['ori_soft']
    #     gpu_list.append('pos') if spe_utils.pos_mode == 'regression' else gpu_list.append('pos_soft')
    ...
    for epoch in range(1, n_epochs + 1):
        for phase in split:
            ...
            for images, targets in loop:
                ...
                # targets = {target.to(device) for target in targets.values()}
                # for gpu in gpu_list:
                #     targets[gpu] = targets[gpu].to(device)
                ...
                with torch.cuda.amp.autocast(enabled=amp):
                    with torch.set_grad_enabled(phase == 'train'):
                        ...
                        if spe_utils.ori_mode == 'keypoints' and spe_utils.pos_mode == 'keypoints':
                            pose = {'keypoints': torch.sigmoid(pred)}
                            # pred = {'keypoints': pred}
                        else:
                            if spe_utils.ori_mode == 'regression':
                                pose = {'ori': torch.nn.functional.normalize(pred[0], p=2, dim=1)}
                            else:
                                pose = {'ori_soft': torch.nn.functional.softmax(pred[0], dim=1)}
                            if spe_utils.pos_mode == 'classification':
                                pose['pos_soft'] = torch.nn.functional.softmax(pred[1], dim=1)
                            else:
                                pose['pos'] = pred[1]

                    # Compute solver
                    loss = spe_loss.compute_loss(pose, targets)
                ...
                pose = {key: value.detach().cpu().numpy() for key, value in pose.items()}
                targets = {key: value.detach().cpu().numpy() for key, value in targets.items()}
                pose.update(spe_utils.decode(pose))
