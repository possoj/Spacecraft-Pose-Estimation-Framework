...
def main():
    ...
    experiment_path = os.path.join('experiments', 'build', 'finn', experiment_name)
    ...
    cfg = load_config(os.path.join(experiment_path, 'config.yaml'))
    ...
    board = import_board(cfg.FINN.BOARD.NAME)

    remote_path = os.path.join(cfg.FINN.BOARD.DEPLOYMENT_FOLDER, experiment_name)
    ...
    # score_finn, score_torch = spe_finn.predict_and_compare(img['torch'], pose, print_results=True)
    ...
    for i, (img, true_pose) in enumerate(data[split['eval'][0]]):
        print(f'\n\n\nImage {i}:')
        true_pose = {key:value.numpy() for key, value in true_pose.items()}  # Convert to numpy
