...
from src.config.build.finn.config import load_config, save_config
...
def main():
    # Monkey patch pdb.set_trace() with our no-op function (seem not to work)
    pdb.set_trace = noop

    seed = 1001
    cfg_root = os.path.join('src', 'config', 'build', 'finn')
    cfg_template = 'exp_'
    ...
    for exp in experiment_list:
        cfg = load_config(os.path.join(cfg_root, exp))
        assert 'brevitas' in cfg.MODEL.BACKBONE.NAME, f'At least the backbone of the model should be quantized with Brevitas'
        exp_id, _ = os.path.splitext(exp.replace(cfg_template, ''))
        save_folder = prepare_directories('experiments', 'build', f'finn/{cfg_template}{exp_id}',
        ...
        params_path = os.path.join(cfg.MODEL.PATH, 'parameters.pt')
        bit_width_path = os.path.join(cfg.MODEL.PATH, 'bit_width.json')
        ...
        # Eval model
        # spe_model = SPETorch(model, torch.device('cpu'), spe_utils)
        ...
        # For now only export the feature extractor
        ...
        auto_fifo = build_cfg.AutoFIFOSizingMethod.LARGEFIFO_RTLSIM if cfg.FINN.FIFO.SIZING_METHOD == 'largefifo_rtlsim' \
            else build_cfg.AutoFIFOSizingMethod.CHARACTERIZE

        cfg = build_cfg.DataflowBuildConfig(
            steps=select_build_steps(cfg.MODEL.BACKBONE.RESIDUAL),
            output_dir=os.path.join(save_folder, 'finn_output'),
            folding_config_file=None,
            synth_clk_period_ns=cfg.FINN.ACCEL.CLK_PERIOD_NS,
            target_fps=int((10**9/cfg.FINN.ACCEL.CLK_PERIOD_NS) / cfg.FINN.ACCEL.TARGET_CYCLES_PER_FRAME),
            board=cfg.FINN.BOARD.NAME,
            shell_flow_type=build_cfg.ShellFlowType.VIVADO_ZYNQ,
            # FIFO
            # Allow very large FIFOs: FIFO nodes with depth larger than 32768 will be split
            split_large_fifos=cfg.FINN.FIFO.SPLIT_LARGE,
            auto_fifo_depths=cfg.FINN.FIFO.AUTO_DEPTH,
            ...
            force_python_rtlsim=cfg.FINN.FIFO.RTL_SIM,
        ...
        # try:
        #     build.build_dataflow_cfg(model_file, cfg)
        #     with open(os.path.join(save_folder, 'build_ok.txt'), 'w+') as f:
        #         f.write('Build completed successfully')
        # except Exception as e:
        #     # Log the exception
        #     logging.basicConfig(filename=os.path.join(save_folder, 'error.log'), level=logging.ERROR)
